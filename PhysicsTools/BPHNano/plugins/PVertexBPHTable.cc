...
void PVertexBPHTable::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& pv : *pvsCol) {
    if (!goodPvCut_(pv) ) { // and i != 0 commented this out to try and test and only store the first pv :)
      i++;
      continue;
    }
    // Remove the dZ cut on the PV for now:
    // bool within_dz = false;
    // for (const pat::CompositeCandidate& dilep : *dileptons)
    // { // Still do this cut, to see if there is some difference
